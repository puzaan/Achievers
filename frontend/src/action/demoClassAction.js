...
import { DEMOCLASS_CREATE_FAIL, DEMOCLASS_CREATE_REQUEST, DEMOCLASS_CREATE_SUCESS, DEMOCLASS_DELETE_FAIL, DEMOCLASS_DELETE_REQUEST, DEMOCLASS_DELETE_SUCCESS, DEMOCLASS_DETAIL_FAIL, DEMOCLASS_DETAIL_ID_FAIL, DEMOCLASS_DETAIL_ID_REQUEST, DEMOCLASS_DETAIL_ID_SUCESS, DEMOCLASS_DETAIL_REQUEST, DEMOCLASS_DETAIL_SUCESS } from 'constants/demoClass'
...
try {
    ...
    const {data} = await axios.post ('http://13.233.151.81:5000/api/scedual/add', {
...
export const DemoClassByid = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(
            `http://13.233.151.81:5000/api/scedual/id/${id}`, config
        );
        dispatch({
...
export const deleteDemoClass = (id) => async(dispatch, getState) => {
    try {
        ...
        await axios.delete(`http://13.233.151.81:5000/api/scedual/id/${id}`,config)
        dispatch({
...
export const DemoClasslist = () => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(`http://13.233.151.81:5000/api/scedual/list`, config);
